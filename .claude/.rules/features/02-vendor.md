...
cars, at what fare, since when — are tracked by the separate Enrollment
feature (`app/features/enrollments/`, no numbered doc, see `CLAUDE.md`),
not by any field on Vendor itself.
...
## Relationships
- One Vendor ↔ many Enrollments (`enrollments.vendor_id`), and through
  ...
  one fixed fare for everything it holds. See the Enrollment feature
...
- `GET /api/v1/vendors/{id}` — retrieve; does **not** include currently
  ...
  fetch `GET /enrollments?vendor_id={id}` separately).
...
- Deleting a Vendor referenced by one or more Enrollments (active or past)
  is restricted: `VendorService.delete` queries `enrollments` for any row
